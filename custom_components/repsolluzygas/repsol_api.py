...
class RepsolLuzYGasSensor():
    ...
    def login(self):
        ...
        response = requests.post('https://gigya.repsolluzygas.com/accounts.login',
    ...
    def get_contracts(self, uid, signature, tstamp):
        ...
        url = 'https://areacliente.repsolluzygas.com/mis-hogares'
        headers = self.get_headers(uid, signature, tstamp, url)

        response = requests.get(
            'https://areacliente.repsolluzygas.com/api/houses', headers=headers, cookies=self.cookies)
        data = response.json()
    ...
    def get_invoices(self, uid, signature, tstamp, house_id, contract_id):
        ...
        response = requests.get('https://areacliente.repsolluzygas.com/api/v2/houses/{}/products/{}/invoices?limit=1'.format(
            house_id, contract_id), headers=headers, cookies=self.cookies)
    ...
    def get_costs(self, uid, signature, tstamp, house_id, contract_id):
        ...
        response = requests.get('https://areacliente.repsolluzygas.com/api/houses/{}/products/{}/consumption/accumulated'.format(
            house_id, contract_id), headers=headers, cookies=self.cookies)
        response = response.json()

        vars = ['consumption']
        attributes = ['totalDays', 'amount',
                      'amountVariable', 'amountFixed', 'start', 'end']
        data = {}
        attributes = {}
        ...
        for attr in attributes:
            attributes[attr] = response.get(attr, 0)
    ...
    def update(self):
        ...
        for contract in contracts['information']:
            if not contract['active']:
                continue
            response = self.get_costs(
                uid, signature, tstamp, contracts['house_id'], contract['contract_id'])
            for var in data:
                data[var] += response[var]

             # 'Consumo': str(consumption, 2).replace('.', ',') + ' kWh',
            attributes = {
                'Precio actual': '{:.2f}'.format(response['amount']).replace('.', ',') + ' €',
                'Tiempo acumulado': str(response['totalDays']) + ' días',
                'Consumo medio': str(round(response['consumption'], 2)).replace('.', ',') + ' kWh',
                'Precio medio': '{:.2f}'.format(round(response['amount'] / response['totalDays'], 2)).replace('.', ',') + ' €',
                'Periodo': datetime.strptime(response['start'], '%Y%m%d').strftime('%d/%m/%Y') + ' - ' + datetime.strptime(response['end'], '%Y%m%d').strftime('%d/%m/%Y'),
                'Gasto fijo': '{:.2f}'.format(response['amountFixed']).replace('.', ',') + ' €',
                'Gasto variable': '{:.2f}'.format(response['amountVariable']).replace('.', ',') + ' €'}

        data['attributes_consumption'] = attributes
    ...
    def get_headers(self, uid, signature, tstamp, url):
        return {
            'Connection': 'keep-alive',
            'Pragma': 'no-cache',
            'Cache-Control': 'no-cache',
